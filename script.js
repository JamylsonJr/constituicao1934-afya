...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── CAIXA DE DETALHE FIXA DA TIMELINE ─────────────────── */
  const detailBox     = document.getElementById('htlDetailBox');
  const detailContent = document.getElementById('htlDetailContent');
  const detailEmpty   = detailBox ? detailBox.querySelector('.htl-detail-empty') : null;

  function showDetail(item) {
    if (!detailBox || !detailContent || !detailEmpty) return;

    const title = item.dataset.title || '';
    const text  = item.dataset.text  || '';
    const extra = item.dataset.extra || '';
    const pills = item.dataset.pills || '';

    let html = `<strong>${title}</strong><p>${text}</p>`;
    if (extra) html += `<em>${extra}</em>`;
    if (pills) {
      const tags = pills.split('|').map(p => `<span>${p}</span>`).join('');
      html += `<div class="htl-pills">${tags}</div>`;
    }
    ...
    // Marcar item ativo
    document.querySelectorAll('.htl-item').forEach(i => i.classList.remove('htl-active'));
    item.classList.add('htl-active');
  }

  document.querySelectorAll('.htl-item[data-year]').forEach(item => {
    // Mouse hover (desktop)
